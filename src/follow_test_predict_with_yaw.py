...
class FollowerRobot:
    def __init__(self):
        ...
        self.odom_sub = rospy.Subscriber('/follow/odom', Odometry, self.odom_callback)
    ...
    def predict_leader_pose(self, dt):
        ...
        # Extract the current and previous positions and orientations
        current_x = self.tag_pose.position.x
        current_z = self.tag_pose.position.z
        current_orientation_q = self.tag_pose.orientation
        prev_x = self.prev_tag_pose.position.x
        prev_z = self.prev_tag_pose.position.z
        prev_orientation_q = self.prev_tag_pose.orientation
        
        # Convert the current and previous quaternions to Euler angles
        _, current_yaw_fliped, _ = euler_from_quaternion([current_orientation_q.x, current_orientation_q.y, current_orientation_q.z, current_orientation_q.w])
        current_yaw = -current_yaw_fliped
        _, prev_yaw_fliped, _ = euler_from_quaternion([prev_orientation_q.x, prev_orientation_q.y, prev_orientation_q.z, prev_orientation_q.w])
        prev_yaw = -prev_yaw_fliped

        # Calculate the linear and angular velocities
        linear_velocity = math.sqrt((current_x - prev_x)**2 + (current_z - prev_z)**2) / dt
        angular_velocity = (current_yaw - prev_yaw) / dt
        
        # Predict the future position and orientation
        predicted_pose = Pose()
        predicted_yaw = current_yaw + angular_velocity * self.prediction_horizon
        predicted_pose.position.x = current_x + linear_velocity * self.prediction_horizon * math.cos(predicted_yaw)
        predicted_pose.position.z = current_z + linear_velocity * self.prediction_horizon * math.sin(predicted_yaw)
        
        # Keep the orientation the same as the predicted orientation
        q = [0.0, 0.0, math.sin(predicted_yaw / 2.0), math.cos(predicted_yaw / 2.0)]
        predicted_pose.orientation.x = q[0]
        predicted_pose.orientation.y = q[1]
        predicted_pose.orientation.z = q[2]
        predicted_pose.orientation.w = q[3]
